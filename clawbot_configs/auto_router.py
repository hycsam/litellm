...
class AutoRouter(CustomLogger):
    ...
    MAX_EMBEDDING_INPUT_TOKENS = 2048
    ...
    def __init__(
        ...
        self._embedding_tokenizer = AutoTokenizer.from_pretrained(self.EMBEDDING_TOKENIZER_NAME)
    ...
    def _extract_text_from_messages(messages: List[Dict[str, Any]]) -> str:
        ...
        messages (where content is a list of content blocks). For multimodal
        messages, only the last text block is returned.
        """
    ...
    def _truncate_text_to_token_limit(self, text: str, max_tokens: int) -> str:
        """
        Truncate text so its token count under the nomic BERT WordPiece tokenizer
        is <= max_tokens. Uses the real tokenizer because the generic fallback severely
        undercounts CJK and code content.
        """
        if not text:
            return text
        ids = self._embedding_tokenizer.encode(text, add_special_tokens=False)
        if len(ids) <= max_tokens:
            return text
        return self._embedding_tokenizer.decode(ids[:max_tokens], skip_special_tokens=True)
    ...
    async def async_pre_routing_hook(
        ...
        message_content = self._truncate_text_to_token_limit(
            message_content, max_tokens=self.MAX_EMBEDDING_INPUT_TOKENS
        )
        verbose_router_logger.info("Message content: %s", message_content)
